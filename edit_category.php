...
// fetch date according to id
//display the data into form
...
print_r($error);
        ...
        if (updateCategory($title, $rank, $status, $updated_at,$id))
         {
            echo 'Category Updated successfully';
        }
         else 
         {
            $error['message'] = "Category Updated failed";
        }
    }
}
$records = getBookCategoryById($id);
if(!$records)
{
...
<body>
    <h1>Update The Book Category</h1>
    ...
    <form action="" method="post">
        <fieldset>
            ...
            <div class="form-group">
                ...
                {
                    ?>
                <input type="radio" id="status_active" name="status" value="1" checked>Active
                <input type="radio" id="status_deactive" name="status" value="0">Deactive
                <?php } 
                else
                {
                    ?>
                     <input type="radio" id="status_active" name="status" value="1" >Active
                     <input type="radio" id="status_deactive" name="status" value="0" checked>Deactive
                <?php } ?> <br>
